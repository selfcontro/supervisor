function buildWorkflowLayout(agents) {
  ...
  workflowGroups.forEach((group) => {
    ...
    const workerSpacing = workerAgents.length > 1 ? 360 : 0
    const groupSpan = Math.max(420, workerAgents.length > 0 ? 420 + workerSpacing * (workerAgents.length - 1) : 420)
    const centerX = cursorX + groupSpan / 2

    if (breakdownAgent) {
      layout[breakdownAgent.id] = { x: centerX, y: breakdownY }
    }
    ...
    if (reviewAgent) {
      layout[reviewAgent.id] = { x: centerX, y: reviewY }
    }

    cursorX += groupSpan + 440
  })
  ...
  extraRuntimeAgents.forEach((agent, index) => {
    layout[agent.id] = { x: Math.max(cursorX, 1120), y: 332 + index * 220 }
  })
